...
- Qué modelo 3D enseña cada ficha lo decide `modelos3d.js`, y lo dibuja con los generadores del juego
  (`js/vista3d/`): la guía no tiene ni un modelo propio.
...
## Los modelos 3D en las fichas

Encargo del dueño (2026-09-13): «en la guía, el arts debería tener los modelos 3D». Las fichas de las
cinco especies (una por casta reclutable), las once del bestiario y las once de sitio (la flora de ese
sitio, con los pesos de `sim/escenarios.js`) enseñan **los modelos del juego**, no un dibujo aparte:
`vista3d/planta-suelta.js` para los árboles, `paisaje.plantar` para las matas, `crearFauna` para los
bichos y `crearColonia` para las hormigas.

`modelos3d.js` dice qué modelo lleva cada ficha y reserva el hueco —de alto fijo, con su botón «Ver en
3D»—; no conoce Three.js y se prueba en node. `visor3d.js` es la maquinaria, y entra con `import()`
solo cuando alguien pulsa: **un renderizador y una escena para toda la guía**, con el lienzo mudándose
al hueco de la ficha que se esté mirando. Sin WebGL, la ficha se queda como estaba.
